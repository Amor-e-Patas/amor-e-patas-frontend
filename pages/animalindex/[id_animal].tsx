  ...
  useEffect(() => {
    async function fetchAPI() {
      try {
        ...
        setSociavel(animais.sociaveis);
        setVivencia(animais.vivencias);
  ...
  return (
    <>
      <div>
        <Navbar />
        <div>
          <p onClick={() => router.back()} className="pointer">
            {" "}
            Voltar
          </p>
          <div className={styles.quadros}>
            <div className={styles.item}>
              <div className={styles.item}>
                <img
                  src={`http://localhost:3333/${animais?.images[0].filepath}`}
            ...
            <div
              ...
              style={{ display: mostrarModalFoto ? "block" : "none" }}
            >
              <div>
                <button
                  onClick={(e) => {
            ...
            <div className={styles.item}>
              ...
              <div className={styles.infos}>
                <div className={styles.item}>
                  <img
                    src="/img/endereco.png"
                    className={styles.endereco}
                    alt=""
                  />{" "}
                  {animais?.desaparecido == "N"
                    ? `Localizado em ${animais?.cidade}, ${estado}:`
                    : `Ultima vez visto em ${animais?.cidade}, ${estado}`}
                </div>
                <br />
              </div>
              <button
              ...
              >
                {animais?.desaparecido == "N" ? `Quero adotar` : `Você me viu?`}
              </button>
              ...
              <div className={styles.listas}>
                <div>
                  <p className={styles.amor}>Temperamento</p>
                  <ul>
                ...
                <div>
                  <p className={styles.amor}>Sociavel com</p>
                  <ul>
                    {sociaveis.map((sociavel, index) => (
                      <li key={index}>{sociavel.descricao}</li>
                    ))}
                  </ul>
                </div>
                
                <div>
                  <p className={styles.amor}>Vive bem em</p>
                  <ul>
                    {vivencias.map((vivencia, index) => (
                      <li key={index}>{vivencia.descricao}</li>
                    ))}
                  </ul>
                </div>
              </div>
          ...
          <div className={styles.quadros}></div>
        </div>
        <Footer />
      </div>
    </>
